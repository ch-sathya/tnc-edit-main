...
import { useCommunityGroups, useJoinCommunityGroup, useLeaveCommunityGroup, useDeleteCommunityGroup } from '@/hooks/useCommunityGroups';
...
const CommunityGroupList: React.FC<CommunityGroupListProps> = ({
  onCreateGroup,
  onGroupSelect
}) => {
  const {
  ...
  if (error) {
    return <RetryHandler error={error} onRetry={async () => {
      await refetch();
    }} title="Failed to load community groups" description="There was an issue loading the community groups. This might be due to a network problem or server issue." maxRetries={3} showNetworkStatus={true}>
  ...
  return <section className="space-y-6" aria-labelledby="community-groups-heading">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        ...
        <Button onClick={onCreateGroup} className="min-h-[44px] w-full sm:w-auto" // Full width on mobile, auto on desktop
      ...
      {isEmpty ? <Card role="region" aria-labelledby="empty-state-heading">
          <CardContent className="pt-6 text-center">
            ...
            <Button onClick={onCreateGroup} className="min-h-[44px]" aria-label="Create your first community group">
              <Plus className="h-4 w-4 mr-2" />
              Create Your First Group
            </Button>
          </CardContent>
        </Card> : <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3" role="grid" aria-label="Community groups list">
          {groups.map(group => <GroupCard key={group.id} group={group} isLoading={loadingStates[group.id] || false} onJoin={() => handleJoinGroup(group.id)} onLeave={() => handleLeaveGroup(group.id, group.name, group.is_owner || false, group.member_count || 0)} onDelete={() => handleDeleteGroup(group.id, group.name)} onSelect={() => onGroupSelect?.(group.id)} onMembers={() => setModals(prev => ({
        ...prev,
        membersModal: {
          open: true,
          group
        }
      }))} onSettings={() => setModals(prev => ({
        ...prev,
        settingsModal: {
          open: true,
          group
      ...
      <ConfirmationDialog open={confirmation.open} onOpenChange={open => setConfirmation(prev => ({
      ...prev,
      open
    }))} onConfirm={executeAction} loading={confirmation.groupId ? loadingStates[confirmation.groupId] : false} {...getConfirmationContent()} />

      {modals.membersModal.group && <GroupMembersModal open={modals.membersModal.open} onOpenChange={open => setModals(prev => ({
      ...prev,
      membersModal: {
        ...prev.membersModal,
        open
      }
    }))} groupId={modals.membersModal.group.id} groupName={modals.membersModal.group.name} />}

      {modals.settingsModal.group && <GroupSettingsModal open={modals.settingsModal.open} onOpenChange={open => setModals(prev => ({
      ...prev,
      settingsModal: {
        ...prev.settingsModal,
        open
